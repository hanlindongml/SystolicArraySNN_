import logging
import math
import configparser  # 有修改
import numpy as np
import torch

from baseline_int.src.utils.utils import ceil_a_by_b, log2, lookup_pandas_dataframe
...
from baseline_int.src.simulator.accelerator import Accelerator
from baseline_int.src.simulator.energy import EnergyTuple
from baseline_int.src.benchmarks.benchmarks import SnnDataLoader

from baseline_int.sram.cacti_sweep import CactiSweep
import os
import pandas
...
class Simulator(object):
    ...
    def __init__(self, config_file, verbose=False, energy_costs=None):
        ...
        self.config = configparser.ConfigParser()  # 有修改
        ...
        dir_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../sram/cacti-master')
        self.sram_obj = CactiSweep(
                bin_file=os.path.join(dir_path, 'cacti'),  # 有修改
    ...
    def get_area(self):
        ...
        ibuf_bank = N
        obuf_bank = M
        wbuf_bits = (pmax * pmax / pmin)  # 8
        ibuf_bits = (pmax * pmax / pmin)
        obuf_bits = 32
        wbuf_word = ceil_a_by_b(wbuf_size, wbuf_bank * wbuf_bits)  # 1
    ...
    def get_energy_cost(self, syn_fn = 'systolic_array_synth.csv'):  # 有修改  # 仿真数据可替换
        ...
        ##################################################
        # Get stats for systolic array
        core_csv = os.path.join('./results', syn_fn)
        core_synth_data = pandas.read_csv(core_csv)  # 有修改
    ...
    def get_FC_cycles(self, Ni, No,
        ...
        """
        total_cycles = self.get_conv_cycles(1, 1, 1, Ni, No, iprec, wprec, batch_size, TW, im2col=True, Speed=Speed)
        return total_cycles


    def get_perf_factor(self, iprec, wprec):
        # pmin: low precision = 8 PE阵列参数
        # pmax: high precision = 8 PE阵列参数
        iprec = max(iprec, self.accelerator.pmin)  # 8
        wprec = max(wprec, self.accelerator.pmin)  # 8
        return int(self.accelerator.pmax / iprec) * int(self.accelerator.pmax / wprec)  # 1

    def get_conv_cycles(self, K, O, S, IC, OC, iprec, wprec, batch_size, TW, im2col, Speed):
        """
        ...
        conv_params = self.accelerator, K, O, S, IC, OC, B, iprec, wprec, im2col, TW, Speed, self.get_energy_cost()
    ...
    def get_cycles(self, op, TW, Speed, im2col=True):
        if isinstance(op, Convolution):
            B, I, _, IC = op.data.shape
            # print('batch size check: ', B)
            ...
            iprec = op.data.dtype.bits
            wprec = op.weights.dtype.bits
            return self.get_FC_cycles(IC, OC, iprec, wprec, B, TW, Speed)