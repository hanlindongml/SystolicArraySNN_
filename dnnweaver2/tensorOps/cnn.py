...
class Convolution(NodeOp):
    def __init__(self, data, weights, bias, node_name, pad='SAME', stride=None, group=1, dtype=FQDtype.FP32, act_fname=None, wgt_fname=None, out_fname=None, in_sparsity_util=0, out_sparsity_util=0):  # 有修改

        if act_fname is not None:  # 有修改
            self.act_fname = act_fname  # 有修改
        if wgt_fname is not None:  # 有修改
            self.wgt_fname = wgt_fname  # 有修改
        if out_fname is not None:  # 有修改
            self.out_fname = out_fname  # 有修改
        self.in_sparsity_util = in_sparsity_util  # 有修改
        self.out_sparsity_util = out_sparsity_util  # 有修改
        # Input data >3D
        self.data = data
...
class MatMul(NodeOp):
    def __init__(self, data, weights, biases, name, dtype=None, act_fname=None, wgt_fname=None, out_fname=None, in_sparsity_util=0, out_sparsity_util=0):  # 有修改
...
def conv2D(i, w, b, name=None, stride=None, pad='SAME', group=1, dtype=None, act_fname=None, wgt_fname=None, out_fname=None, in_sparsity_util=0, out_sparsity_util=0):  # 有修改
    g = get_default_graph()
    op = Convolution(i, w, b, name, stride=stride, pad=pad, group=group, dtype=dtype, act_fname=act_fname, wgt_fname=wgt_fname, out_fname=out_fname, in_sparsity_util=in_sparsity_util, out_sparsity_util=out_sparsity_util)  # 有修改
    return typecast(op.output_tensors, dtype)
...
def matmul(i, w, b, name=None, dtype=None, act_fname=None, wgt_fname=None, out_fname=None, in_sparsity_util=0, out_sparsity_util=0):  # 有修改
    g = get_default_graph()
    op = MatMul(i, w, b, name=name, dtype=dtype, act_fname=act_fname, wgt_fname=wgt_fname, out_fname=out_fname, in_sparsity_util=in_sparsity_util, out_sparsity_util=out_sparsity_util)  # 有修改
    return typecast(op.output_tensors, dtype)
