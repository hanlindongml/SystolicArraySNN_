...
class Simulator(object):
    ...
    def get_FC_cycles(self, Ni, No,
                      iprec, wprec,
                      batch_size=1):  # 获取全连接层需要的时钟周期数
        """
        Get number of cycles required for Fully-Connected Layer.

        args:
            Ni: Input neurons 输入神经元个数
            No: Output neurons 输出神经元个数
            batch_size: Batch size for FC layer 批处理大小
            iprec: Precision for activations (bits) 激活的精度
            wprec: Precision for weights (bits) 权重的精度
    ...
    def get_conv_cycles(self, K, O, S, IC, OC, iprec, wprec, batch_size=1, im2col=False):
        ...
        args:
            ...
            batch_size: Batch size for the layer TW大小
        ...
        best_instructions, best_tiling, best_order, _, _ = optimize_for_order(conv_params)  # 获取最佳指令，平铺方式和顺序
        stats = get_stats_fast(conv_params, best_tiling, best_order, verbose=False)  # 得到统计指标
        ...
        num_ops = O * O * K * K * IC * OC * B  # 需要进行的操作数
        ...
        cycles_per_batch = ceil_a_by_b(best_cycles, batch_size)  # cycles/batch_size并向上取整
        self.logger.debug('Total Cycles per batch: {:,}'.format(cycles_per_batch))
        ops_per_cycle = float(num_ops) / best_cycles  # 每一时钟周期的操作数
        self.logger.debug('Ops/Cycle: {:,.2f}'.format(ops_per_cycle))
        ops_per_cycle_per_pe = float(ops_per_cycle) / (self.accelerator.N * self.accelerator.M)  # 每PE每时钟周期的操作数。N * M为PE数量.
